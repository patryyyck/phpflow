...
final class HtmlExporterTest extends TestCase
{
    public function testItEmbedsVersionedGraphJsonInSelfContainedHtml(): void
    {
        ...
        self::assertStringContainsString('id="minimap"', $html);
        self::assertStringContainsString('id="toggle-minimap"', $html);
        self::assertStringContainsString('Minimap on', $html);
        self::assertStringContainsString("button.textContent=hidden?'Minimap off':'Minimap on'", $html);
        self::assertStringContainsString('function renderMinimap(ids)', $html);
        self::assertStringContainsString('function navigateFromMinimap(clientX,clientY)', $html);
        ...
        self::assertStringContainsString('function criticalPath(id)', $html);
        self::assertStringContainsString("id:'arrow-critical'", $html);
        self::assertStringContainsString('Critical path', $html);
        self::assertStringContainsString('critical-highlighted', $html);
        ...
        self::assertStringContainsString('maxEdges=1200', $html);
        self::assertStringContainsString('document.createDocumentFragment()', $html);
        self::assertStringContainsString("pathOnly=Boolean(pathFromEntryPoint(n.id))", $html);
