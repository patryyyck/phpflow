...
#[AsCommand(
    name: 'impact:exception',
    description: 'Finds entry points that can throw an exception.',
)]
final class ImpactExceptionCommand extends Command
{
    public function __construct(
        ...
        private readonly FindExceptionImpact $impactFinder,
    ...
    protected function configure(): void
    {
        $this
            ...
            ->addArgument(
                ...
                'Exception FQCN or short class name.',
            );
    }
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        $exception = (string) $input->getArgument('exception');

        $project = $this->scanProject->scan($path);
        $analysis = $this->analyzer->analyze($project);
        $graph = $this->graphBuilder->build($analysis);
        $impacts = $this->impactFinder->find($graph, $exception);

        $io = new SymfonyStyle($input, $output);
        $io->title(sprintf('Entry points that can throw %s', $exception));
