...
let scale=1,tx=0,ty=0,drag=null,selected=null,focusRoot=null,directOnly=false,pathOnly=false,effectsOnly=false,criticalOnly=false,searchQuery='',searchMatches=[],explorePreset='all',hideTechnical=false,minimapState=null,showLanes=true;
...
function criticalPath(id){
 const upstream=pathFromEntryPoint(id),downstream=pathsToEffects(id);
 if(!upstream&&!downstream)return null;
 const nodes=new Set([id]),edgeKeys=new Set();
 if(upstream){upstream.nodes.forEach(x=>nodes.add(x));upstream.edgeKeys.forEach(x=>edgeKeys.add(x))}
 if(downstream){downstream.nodes.forEach(x=>nodes.add(x));downstream.edgeKeys.forEach(x=>edgeKeys.add(x))}
 return {nodes,edgeKeys,entry:upstream?.entry||null,effects:downstream?.effects||new Set()};
}
...
function revealNode(id){
 focusRoot=null;directOnly=false;pathOnly=false;effectsOnly=false;criticalOnly=false;explorePreset='all';hideTechnical=false;
...
function selectSearchResult(n){
 revealNode(n.id);selected=n.id;effectsOnly=false;criticalOnly=false;pathOnly=Boolean(pathFromEntryPoint(n.id));showDetails(n);render();centerOn(n.id);
...
function render(){
 ...
 effectMarker.appendChild(el('path',{d:'M 0 0 L 10 5 L 0 10 z',fill:'#ea580c'}));defs.appendChild(effectMarker);
 const criticalMarker=el('marker',{id:'arrow-critical',viewBox:'0 0 10 10',refX:'9',refY:'5',markerWidth:'7',markerHeight:'7',orient:'auto-start-reverse'});
 criticalMarker.appendChild(el('path',{d:'M 0 0 L 10 5 L 0 10 z',fill:'#7c3aed'}));defs.appendChild(criticalMarker);svg.appendChild(defs);
 ...
 let highlight=new Set(),selectedPath=selected&&pathOnly?pathFromEntryPoint(selected):null,selectedEffects=selected&&effectsOnly?pathsToEffects(selected):null,selectedCritical=selected&&criticalOnly?criticalPath(selected):null;
 ...
 graph.edges.filter(e=>ids.has(e.source)&&ids.has(e.target)).forEach(e=>{
  ...
  if(criticalOnly&&selectedCritical)cls+=selectedCritical.edgeKeys.has(edgeKey(e))?' critical-highlighted':' dimmed';
  else if(effectsOnly&&selectedEffects)cls+=selectedEffects.edgeKeys.has(edgeKey(e))?' effect-highlighted':' dimmed';
  ...
  if(edgeText){
   const label=el('text',{x:midX,y:(a.y+b.y)/2-6,class:`edge-label${asyncBoundary?' async-boundary':''}${cls.includes(' highlighted')?' highlighted':''}${cls.includes(' path-highlighted')?' path-highlighted':''}${cls.includes(' effect-highlighted')?' effect-highlighted':''}${cls.includes(' critical-highlighted')?' critical-highlighted':''}${cls.includes(' dimmed')?' dimmed':''}`});
 ...
 graph.nodes.filter(n=>ids.has(n.id)).forEach(n=>{
  ...
  if(criticalOnly&&selectedCritical)cls+=selectedCritical.nodes.has(n.id)?' critical-highlighted':' dimmed';
  else if(effectsOnly&&selectedEffects)cls+=selectedEffects.nodes.has(n.id)?' effect-highlighted':' dimmed';
  ...
  g.addEventListener('click',ev=>{ev.stopPropagation();selected=n.id;directOnly=false;effectsOnly=false;criticalOnly=false;pathOnly=Boolean(pathFromEntryPoint(n.id));showDetails(n);render()});viewport.appendChild(g);
 });
 renderMinimap(ids);
 document.getElementById('stats').textContent=`Schema ${graph.schemaVersion} · ${ids.size}/${graph.nodes.length} nodes · ${graph.edges.length} edges${focusRoot?' · focused':''}${pathOnly?' · entry path':''}${effectsOnly?' · effects path':''}${criticalOnly?' · critical path':''}${explorePreset!=='all'?` · ${explorePreset.replace('_',' ')}`:''}${hideTechnical?' · technical hidden':''}`;
}
function showDetails(n){
 const direct=graph.edges.filter(e=>e.source===n.id||e.target===n.id),entry=entryPointFor(n.id),children=(outgoing.get(n.id)||[]).length,effectPaths=pathsToEffects(n.id),effectCount=effectPaths?.effects.size||0,critical=criticalPath(n.id);
 ...
 effectsOnly=!effectsOnly;directOnly=false;pathOnly=false;criticalOnly=false;
 ...
 const cn=document.getElementById('critical-node');if(cn)cn.onclick=()=>{
 criticalOnly=!criticalOnly;directOnly=false;pathOnly=false;effectsOnly=false;
 if(criticalOnly&&critical){
  critical.nodes.forEach(id=>{const node=nodeById.get(id);if(node)enabled.add(node.type)});
  syncFilters();
 }
 showDetails(n);render();
};
 const ep=document.getElementById('entry-node');if(ep)ep.onclick=()=>{selected=entry.id;focusRoot=null;directOnly=false;pathOnly=false;effectsOnly=false;criticalOnly=false;showDetails(entry);render();centerOn(entry.id)};
 ...
 const cf=document.getElementById('clear-focus');if(cf)cf.onclick=()=>{focusRoot=null;directOnly=false;pathOnly=false;effectsOnly=false;criticalOnly=false;fit()};
}
...
svg.addEventListener('click',()=>{selected=null;directOnly=false;pathOnly=false;effectsOnly=false;criticalOnly=false;document.getElementById('details').innerHTML='<div class="empty">Select a node in the graph.</div>';render()});
